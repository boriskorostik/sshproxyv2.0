...
def free_proxy_port(show_dialogs=True):
...
def hard_disconnect(show_dialogs=True):
    global ssh_process, current_host
    ...
    ssh_process = None
    current_host = None
    free_proxy_port(show_dialogs=show_dialogs)
    hide_suggestions()
    status_label.config(text="Не подключено", fg="#5B6B7A")


def disconnect():
    if FALLBACK_HOST not in all_ssh_hosts:
        messagebox.showerror(
            "Не найден testclvr",
            f"Alias {FALLBACK_HOST} не найден в ~/.ssh/config.",
            parent=root,
        )
        return

    log(f"🔄 Переключаюсь на {FALLBACK_HOST}...\n")
    hard_disconnect(show_dialogs=False)
    apply_host(FALLBACK_HOST)
    root.after(200, lambda: start_ssh_proxy(FALLBACK_HOST))


def on_window_close():
    log("🔧 Закрываю окно: снимаю SSH-прокси и освобождаю localhost:3000...\n")
    hard_disconnect(show_dialogs=False)
    root.destroy()


root = tk.Tk()
root.title("SSHProxy Leonid")
...
log(f"Кнопка 'Отключиться' в этой версии переключает на {FALLBACK_HOST}.\n\n")
